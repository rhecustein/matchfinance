<x-app-layout>
    <x-slot name="header">Edit Sub Category</x-slot>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <nav class="flex items-center space-x-2 text-sm">
                ...
                <span class="text-white font-semibold">Edit {{ $subCategory->name }}</span>
            </nav>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 border border-slate-700 shadow-xl">
                    <div class="mb-8">
                        <div class="flex items-center space-x-4 mb-4">
                            <div class="w-14 h-14 rounded-xl flex items-center justify-center" style="background: linear-gradient(135deg, {{ $subCategory->category->color }}, {{ $subCategory->category->color }}cc);">
                                <i class="fas fa-layer-group text-white text-2xl"></i>
                            </div>
                            <div>
                                <h2 class="text-2xl font-bold text-white">Edit Sub Category</h2>
                                <p class="text-gray-400">Update sub category information</p>
                            </div>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('sub-categories.update', $subCategory) }}" class="space-y-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div>
                            ...
                            <select name="category_id" id="category_id" required class="w-full px-4 py-3 bg-slate-900/50 border border-slate-700 rounded-xl text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $subCategory->name) }}" required maxlength="100" 
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" maxlength="1000" 
                                      class="w-full px-4 py-3 bg-slate-900/50 border border-slate-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">{{ old('description', $subCategory->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-2">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                                </p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="priority" id="priority" required class="w-full px-4 py-3 bg-slate-900/50 border border-slate-700 rounded-xl text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                                    @for($i = 10; $i >= 1; $i--)
                                        <option value="{{ $i }}" {{ old('priority', $subCategory->priority) == $i ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $subCategory->sort_order) }}" min="0" 
            ...
            <div class="space-y-6">
                <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700 shadow-xl">
                    <h3 class="text-lg font-bold text-white mb-4">Sub Category Info</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between text-sm py-3 border-b border-slate-700">
                            <span class="text-gray-400">Category</span>
                            <span class="text-white font-semibold">{{ $subCategory->category->name }}</span>
                        </div>
                        <div class="flex justify-between text-sm py-3 border-b border-slate-700">
                            <span class="text-gray-400">Type</span>
                            <span class="text-white font-semibold">{{ $subCategory->category->type->name }}</span>
                        </div>
                        <div class="flex justify-between text-sm py-3 border-b border-slate-700">
                            <span class="text-gray-400">Keywords</span>
                            <span class="text-white font-semibold">{{ $subCategory->keywords()->count() }}</span>
                        </div>
                        <div class="flex justify-between text-sm py-3 border-b border-slate-700">
                            <span class="text-gray-400">Created</span>
                            <span class="text-white font-semibold">{{ $subCategory->created_at->format('M d, Y') }}</span>
                        </div>
                        <div class="flex justify-between text-sm py-3">
                            <span class="text-gray-400">Updated</span>
                            <span class="text-white font-semibold">{{ $subCategory->updated_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
                ...
                @if($subCategory->keywords()->count() == 0)
                    <div class="bg-gradient-to-br from-red-900/20 to-slate-900 rounded-2xl p-6 border border-red-500/30 shadow-xl">
                        ...
                        <p class="text-gray-400 text-sm mb-4">Delete this sub category permanently.</p>
                        <button onclick="confirmDelete()" class="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-xl font-semibold transition-all">
                            <i class="fas fa-trash mr-2"></i>Delete Sub Category
                        </button>
                    </div>
                @else
                    <div class="bg-gradient-to-br from-yellow-900/20 to-slate-900 rounded-2xl p-6 border border-yellow-500/30 shadow-xl">
                        ...
                        <p class="text-gray-400 text-sm">Has {{ $subCategory->keywords()->count() }} keywords.</p>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-8 border border-slate-700">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-gray-400">Delete <strong class="text-white">{{ $subCategory->name }}</strong>?</p>
                    </div>
