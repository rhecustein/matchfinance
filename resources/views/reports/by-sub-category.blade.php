<x-app-layout>
    <x-slot name="header">By Sub Category Report - {{ $year }}</x-slot>
    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <div class="flex items-center space-x-3 mb-2">
                    ...
                    <h2 class="text-3xl font-bold text-white">By Sub Category Report</h2>
                </div>
        ...
        <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700 shadow-xl mb-8">
            <form action="{{ route('reports.by-sub-category') }}" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    <label class="text-gray-400 text-sm mb-2 block">Year</label>
                    <select name="year" required class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-yellow-500">
                ...
                <div>
                    ...
                    <select name="bank_id" class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-yellow-500">
                ...
                <div>
                    ...
                    <select name="category_id" class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-yellow-500">
                ...
                <div>
                    ...
                    <select name="transaction_type" class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-yellow-500">
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 rounded-lg transition">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-gradient-to-br from-green-900/20 to-slate-900 rounded-2xl p-6 border border-green-500/30 shadow-xl">
                <p class="text-gray-400 text-sm mb-1">Sub Categories</p>
                <p class="text-white text-3xl font-bold">{{ $subCategories->count() }}</p>
            </div>
            <div class="bg-gradient-to-br from-purple-900/20 to-slate-900 rounded-2xl p-6 border border-purple-500/30 shadow-xl">
                <p class="text-gray-400 text-sm mb-1">Avg per Sub Category</p>
                <p class="text-white text-3xl font-bold">Rp {{ $subCategories->count() > 0 ? number_format($grandTotal['total'] / $subCategories->count(), 0, ',', '.') : 0 }}</p>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-900/50">
                        <tr>
                            <th class="px-6 py-4 text-left text-white font-bold border-b border-slate-700 sticky left-0 bg-slate-900/50 z-10 min-w-[300px]">
                                Sub Category (Category > Type)
                            </th>
                            ...
                            @endforeach
                            <th class="px-6 py-4 text-center text-white font-bold border-b border-slate-700 bg-yellow-900/30 min-w-[180px]">
                                Total
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($subCategories as $subCategory)
                            <tr class="border-b border-slate-700/50 hover:bg-slate-800/50 transition">
                                ...
                                @foreach($months as $monthData)
                                    @php
                                        $subCategoryData = $monthData['subCategories'][$subCategory->id] ?? ['total' => 0, 'count' => 0];
                                    @endphp
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex flex-col space-y-1">
                                            <span class="text-white font-bold">
                                                Rp {{ number_format($subCategoryData['total'], 0, ',', '.') }}
                                            </span>
                                            <span class="text-gray-400 text-xs">
                                                {{ number_format($subCategoryData['count']) }} txn
                                            </span>
                                        </div>
                                    </td>
                                @endforeach
                                <td class="px-6 py-4 text-center bg-yellow-900/20">
                                    @php
                                        $subCategoryTotal = $grandTotal['subCategories'][$subCategory->id] ?? ['total' => 0, 'count' => 0];
                                    @endphp
                                    <div class="flex flex-col space-y-1">
                                        <span class="text-white font-bold">
                                            Rp {{ number_format($subCategoryTotal['total'], 0, ',', '.') }}
                                        </span>
                                        <span class="text-gray-400 text-xs">
                                            {{ number_format($subCategoryTotal['count']) }} txn
                                        </span>
                                    </div>
                                </td>
                            </tr>
                        ...
                        {{-- Grand Total Row --}}
                        <tr class="bg-yellow-900/30 border-t-2 border-yellow-500/50">
                            <td class="px-6 py-4 text-white font-bold sticky left-0 bg-yellow-900/50 z-10">
                                GRAND TOTAL
                            </td>
                            @foreach($months as $monthData)
                                <td class="px-6 py-4 text-center">
                            ...
                            @endforeach
                            <td class="px-6 py-4 text-center bg-yellow-900/40">
                                <div class="flex flex-col space-y-1">
                                    <span class="text-white font-bold text-base">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700 shadow-xl">
                <h3 class="text-white font-bold text-lg mb-4">
                    <i class="fas fa-chart-bar mr-2"></i>Top 10 Sub Categories
                </h3>
                <canvas id="subCategoryBarChart" height="100"></canvas>
            </div>

            {{-- Line Chart --}}
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700 shadow-xl">
                <h3 class="text-white font-bold text-lg mb-4">
                    <i class="fas fa-chart-line mr-2"></i>Top 5 Monthly Trend
                </h3>
                <canvas id="subCategoryTrendChart" height="100"></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const barCtx = document.getElementById('subCategoryBarChart').getContext('2d');
        
        const subCategoriesData = @json($grandTotal['subCategories'] ?? []);
        ...
        sortedSubCategories.forEach(item => {
            const subCat = @json($subCategories).find(s => s.id == item[0]);
            if (subCat) {
                barLabels.push(subCat.name);
                barData.push(item[1].total);
            }
        });
        ...
        // Line Chart - Top 5 Trend
        const trendCtx = document.getElementById('subCategoryTrendChart').getContext('2d');
        const months = @json(array_column($months, 'month'));
        const datasets = [];
        
        const top5SubCategories = sortedSubCategories.slice(0, 5);
        
        top5SubCategories.forEach(item => {
            const subCat = @json($subCategories).find(s => s.id == item[0]);
            if (subCat) {
                const monthlyData = @json(array_column(array_column($months, 'subCategories'), item[0]));
                datasets.push({
                    label: subCat.name,
                    data: monthlyData.map(d => d ? d.total : 0),
                    borderWidth: 2,
                    tension: 0.4
                });
            }
