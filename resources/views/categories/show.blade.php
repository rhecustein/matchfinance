    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <nav class="flex items-center space-x-2 text-sm">
                ...
                <a href="{{ route('types.show', $category->type) }}" class="text-gray-400 hover:text-white transition">
                    {{ $category->type->name }}
                </a>
                <i class="fas fa-chevron-right text-gray-600 text-xs"></i>
        ...
        {{-- Header Card --}}
        <div class="rounded-2xl p-8 mb-8 shadow-2xl" style="background: linear-gradient(135deg, {{ $category->color }}, {{ $category->color }}cc);">
            <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
                ...
                <div class="flex-1 text-center md:text-left">
                    ...
                    @if($category->description)
                        <p class="text-white/90 mb-4">{{ $category->description }}</p>
                    @endif
                ...
                <a href="{{ route('categories.edit', $category) }}" class="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl font-semibold transition-all backdrop-blur-sm flex items-center space-x-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700 shadow-xl hover:scale-105 transition-transform">
                ...
                <p class="text-white text-3xl font-bold">{{ $stats['total_subcategories'] }}</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 border border-slate-700 shadow-xl">
            <div class="flex items-center justify-between mb-6">
                ...
                @if($category->subCategories->count() > 0)
                    <a href="{{ route('sub-categories.index') }}?category_id={{ $category->id }}" class="text-blue-500 hover:text-blue-400 text-sm font-semibold">
                        View All →
                    </a>
                @endif
            </div>
            ...
            @if($category->subCategories->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($category->subCategories as $subCategory)
                        <a href="{{ route('sub-categories.show', $subCategory) }}" class="bg-slate-900/50 rounded-xl p-6 hover:bg-slate-800 transition group block">
                            <div class="flex items-start justify-between mb-3">
                                <div class="w-10 h-10 rounded-lg flex items-center justify-center" style="background-color: {{ $category->color }}20;">
                                    <i class="fas fa-folder-open" style="color: {{ $category->color }};"></i>
                                </div>
                                <div class="flex flex-col items-end gap-1">
                                    ...
                                    <span class="px-2 py-1 bg-blue-600/20 text-blue-400 rounded text-xs font-semibold">
                                        {{ $subCategory->keywords_count }} keywords
                                    </span>
                                </div>
                            </div>
                            ...
                            @if($subCategory->description)
                                <p class="text-gray-400 text-xs line-clamp-2">{{ $subCategory->description }}</p>
                            @endif
                        </a>
                    @endforeach
                </div>
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-400 text-lg mb-4">No sub categories yet</p>
                    <a href="{{ route('sub-categories.create') }}?category_id={{ $category->id }}" class="inline-flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold transition-all">
