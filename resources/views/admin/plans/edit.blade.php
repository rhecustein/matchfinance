<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form action="{{ route('admin.plans.update', $plan) }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  rows="3"
                                  class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                  placeholder="Describe this plan...">{{ old('description', $plan->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="max_products" 
                               id="max_products" 
                               value="{{ old('max_products', $plan->features['max_products'] ?? -1) }}"
                               min="-1"
                               required
                               class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               placeholder="-1">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               ...
                               id="max_transactions" 
                               value="{{ old('max_transactions', $plan->features['max_transactions'] ?? -1) }}"
                               min="-1"
                               required
                               class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               placeholder="-1">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="max_storage_mb" 
                               id="max_storage_mb" 
                               value="{{ old('max_storage_mb', $plan->features['max_storage_mb'] ?? -1) }}"
                               min="-1"
                               required
                               class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               placeholder="-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-start gap-3 p-4 bg-slate-900/50 rounded-lg">
                        <input type="checkbox" 
                               ...
                               id="bank_statements" 
                               value="1"
                               {{ old('bank_statements', $plan->features['bank_statements'] ?? false) ? 'checked' : '' }}
                    ...
                    <div class="flex items-start gap-3 p-4 bg-slate-900/50 rounded-lg">
                        <input type="checkbox" 
                               ...
                               id="advanced_reports" 
                               value="1"
                               {{ old('advanced_reports', $plan->features['advanced_reports'] ?? false) ? 'checked' : '' }}
                               class="w-5 h-5 mt-0.5 bg-slate-700 border-slate-600 rounded text-blue-600 focus:ring-2 focus:ring-blue-500">
                        <div>
                    ...
                    <div class="flex items-start gap-3 p-4 bg-slate-900/50 rounded-lg">
                        <input type="checkbox" 
                               ...
                               id="api_access" 
                               value="1"
                               {{ old('api_access', $plan->features['api_access'] ?? false) ? 'checked' : '' }}
                               class="w-5 h-5 mt-0.5 bg-slate-700 border-slate-600 rounded text-blue-600 focus:ring-2 focus:ring-blue-500">
                        <div>
                    ...
                    <div class="flex items-start gap-3 p-4 bg-slate-900/50 rounded-lg md:col-span-2">
                        <input type="checkbox" 
                               ...
                               id="custom_branding" 
                               value="1"
                               {{ old('custom_branding', $plan->features['custom_branding'] ?? false) ? 'checked' : '' }}
                               class="w-5 h-5 mt-0.5 bg-slate-700 border-slate-600 rounded text-blue-600 focus:ring-2 focus:ring-blue-500">
                        <div>
            ...
            <!-- Warning if has active subscribers -->
            @if($plan->active_subscriptions_count > 0)
            <div class="bg-yellow-900/20 border border-yellow-600 rounded-2xl p-6">
                <div class="flex items-start gap-3">
                    <i class="fas fa-exclamation-triangle text-yellow-500 text-xl mt-1"></i>
                    <div>
                        <h4 class="text-yellow-400 font-semibold mb-1">Warning: Active Subscribers</h4>
                        <p class="text-yellow-300 text-sm">
                            This plan has {{ $plan->active_subscriptions_count }} active subscriber(s). 
                            Changes to limits and features will affect existing subscribers immediately.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Action Buttons -->
            <div class="flex items-center justify-between">
                <a href="{{ route('admin.plans.show', $plan) }}" class="px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition font-semibold">
